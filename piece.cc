...
vector<pair<int, int>> Pawn::validMoves() const {
  ...
  if (isWhite) {
    if (row-1 >= 0 && b.board->at(row-1, col) == nullptr) moves.emplace_back(row-1, col);
    if (unmoved && row-2 >= 0 && b.board->at(row-2, col) == nullptr) moves.emplace_back(row-2, col);
    // TODO: en passant
  }
  else {
    if (row+1 <= 7 && b.board->at(row+1, col) == nullptr) moves.emplace_back(row+1, col);
    if (unmoved && row+2 <= 7 && b.board->at(row+2, col) == nullptr) moves.emplace_back(row+2, col);
    // TODO: en passant
  }
...
vector<pair<int, int>> King::validMoves() const {
  ...
  for (int r = -1; r <= 1; ++r) {
    ...
    for (int c = -1; c <= 1; ++c) {
      int newCol = col+c;
      if (newRow >= 0 && newRow <= 7 && newCol >= 0 && newCol <= 7 && b.board->at(newRow,newCol) == nullptr) {
  ...
  if (canCastle) {
    ...
    if (b.board->at(row, 0)->getSymbol() == (isWhite ? 'R' : 'r')) {
      bool flag = true;
      for (int i = 1; i < col ; ++i) {
        if (b.board->at(row, i) != nullptr) {
          flag = false;
          break;
        }
      }
    ...
    if (b.board->at(row,7)->getSymbol() == (isWhite ? 'R' : 'r')) {
      bool flag = true;
      for (int i = 6; i > col ; --i) {
        if (b.board->at(row, i) != nullptr) {
          flag = false;
...
vector<pair<int, int>> Bishop::validMoves() const {
  ...
  int r = row, c = col;
  while (++r <= 7 && ++c <= 7) {
    if (b.board->at(r,c) != nullptr) break;
...
vector<pair<int, int>> Rook::validMoves() const {
  ...
  for (int c = col+1; c <= 7; ++c) {
    if (b.board->at(row,c) != nullptr) break;
  ...
  for (int r = row+1; r <= 7; ++r) {
    if (b.board->at(row,col) != nullptr) break;
    moves.emplace_back(r, col);
  }
  for (int r = row-1; r >= 0; --r) {
    if (b.board->at(r,col) != nullptr) break;
    moves.emplace_back(r, col);
  }
...
vector<pair<int, int>> Knight::validMoves() const {
  ...
  if (row+1 <= 7 && col+2 <= 7 && b.board->at(row+1,col+2) == nullptr) moves.emplace_back(row+1, col+2);
  if (row+2 <= 7 && col+1 <= 7 && b.board->at(row+2, col+1) == nullptr) moves.emplace_back(row+2, col+1);
  if (row+1 <= 7 && col-2 >= 0 && b.board->at(row+1, col-2) == nullptr) moves.emplace_back(row+1, col-2);
  if (row+2 <= 7 && col-1 >= 0 && b.board->at(row+2, col-1) == nullptr) moves.emplace_back(row+2, col-1);
  if (row-1 >= 0 && col+2 <= 7 && b.board->at(row-1, col+2) == nullptr) moves.emplace_back(row-1, col+2);
  if (row-2 >= 0 && col+1 <= 7 && b.board->at(row-2, col+1) == nullptr) moves.emplace_back(row-2, col+1);
  if (row-1 >= 0 && col-2 >= 0 && b.board->at(row-1, col-2) == nullptr) moves.emplace_back(row-1, col-2);
  if (row-2 >= 0 && col-1 >= 0 && b.board->at(row-2, col-1) == nullptr) moves.emplace_back(row-2, col-1);
